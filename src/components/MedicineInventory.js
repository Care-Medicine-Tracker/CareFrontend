...
export class MedicineInventory extends Component
{
  static displayName = MedicineInventory.name;
  ...
  async populateItems()
  {
    fetch(`${window.MEDICINE_INVENTORY_ITEMS_API_URL}`)
      .then(response => { 
  ...
  async deleteItem(id)
  {
    ...
    if (confirmDeletion)
    {
      fetch(`${window.MEDICINE_INVENTORY_ITEMS_API_URL}/${id}`, {
        method: 'delete',
        headers: {
  ...
  render()
  {
    ...
    return (
      <div>
        <h1 id="tabelLabel" >Medicine Inventory Items</h1>
        {contents}
      </div>
    );
  }
