...
export class UserMedicineInventory extends Component
{
  static displayName = UserMedicineInventory.name;
  ...
  async populateItems()
  {
    ...
    fetch(`${window.USER_MEDICINE_INVENTORY_ITEMS_API_URL}?userId=${this.state.userId}`)
  ...
  renderInputs()
  {
    return <Form inline >
      ...
      <Button className="mb-2" variant="primary" onClick={() => this.populateItems()}>Get User Medicine Inventory</Button>
    </Form>;
  }
                    ...
                    {!this.state.items || this.state.items.length <= 0 ?
                      ...
                      : this.state.items.map(item => (
                        <tr key={item.catalogItemId}>
                          ...
                          <td>
                            {`${item.intakeHour} : ${item.intakeMinute}`}
                          </td>
                        </tr>
                      ))}
                  </tbody>
  ...
  render()
  {
    return (
      <div>
        <h1 id="tabelLabel" >User Medicine Inventory</h1>
        {this.renderInputs()}
